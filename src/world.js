...
export function createHouse(){
 ...
 wall(-7,0,.4,5,19); wall(7,-7,.4,5,9); wall(7,12,.4,5,16); wall(0,11,14,5,.35);
...
function makeDoor(x,z,rot){
 ...
 const c={x1:x-1.3,x2:x+1.3,z1:z-.12,z2:z+.12,source:d,door:true}; d.userData.collider=c; colliders.push(c); doors.push(d); return d;
}
export function updateDoors(dt){
 for(const d of doors){const u=d.userData; u.openProgress+=(u.target-u.openProgress)*Math.min(1,dt*10); d.rotation.y=u.baseRot+u.openProgress*Math.PI/2;}
}
export function isBlocked(x,z,r=.45){
 for(const c of colliders){if(c.door && c.source.userData.openProgress>.65)continue;if(x+r>c.x1&&x-r<c.x2&&z+r>c.z1&&z-r<c.z2)return true;}
 return false;
}
