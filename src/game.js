...
import { createHouse, doors, updateDoors } from './world.js';
...
function doorInteract(){const p=controls.getObject().position;let best=null,bd=3;for(const d of doors){const dd=Math.hypot(p.x-d.position.x,p.z-d.position.z);if(dd<bd){bd=dd;best=d;}}if(best){best.userData.open=!best.userData.open;best.userData.target=best.userData.open?1:0;neighbor.hear(best.position,12);}}
