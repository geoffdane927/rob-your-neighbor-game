import { scene, camera, renderer, world, controls, state, keys, clock } from './core.js';
...
addEventListener('keydown', e => { keys[e.code] = true; if (e.code === 'KeyE') state.carried ? dropFurniture() : interactFurniture(); });
...
addEventListener('keydown', e => { if (e.code === 'KeyF') doorInteract(); });
...
function updatePlayer(dt) {
  ...
  if(v.lengthSq()>0){ v.normalize().applyQuaternion(camera.quaternion); v.y=0; v.normalize(); p.x+=v.x*speed*dt; p.z+=v.z*speed*dt; if(sprint) neighbor.hear(p,4); }
  p.x=Math.max(-21.5,Math.min(21.5,p.x)); p.z=Math.max(-18.5,Math.min(18.5,p.z)); p.y=1.65;
}
function endGame(){ state.gameEnded=true; document.getElementById('gameover').style.display='flex'; controls.unlock(); }

function animate(){ requestAnimationFrame(animate); const dt=Math.min(.033,clock.getDelta()); updatePlayer(dt); world.step(1/60,dt,3); updateFurniture(); updateNeighbor(dt,endGame); checkDelivery(); renderer.render(scene,camera); }
animate();
